function postout = ...
            PostProcess_sim(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_tau,MCMC_pi,MCMC_theta,theta,model,wavespecsc,wavespecsy,pcaspecsx,twosample,get_sigma,sampleU,MCMC_U)
    %% Outputs:
...
%     MCMC_pi       = res.MCMC_pi;
    ...
    if twosample == 0;
        ...
        for i = 1:B;
            ...
            Beta(i,:)   = reshape(Betai,1,Tx*Tx); % T by Tx
    ...
    if length(delt) == 1;
        postout.psi     = reshape(psi,Tx,Tx); %check if T, Tx or Tx, T
    end;
    if length(delt) == 2;
