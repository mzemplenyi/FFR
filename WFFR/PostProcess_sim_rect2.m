function postout = ...
            PostProcess_sim_rect2(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_tau,MCMC_pi,MCMC_theta,theta,model,wavespecsc,wavespecsy,pcaspecsx,twosample,get_sigma,sampleU,MCMC_U)
    ...
    %% Test: comment this chunk out when no longer loading the postMCMC object
%     MCMC_beta     = postMCMC.MCMC_beta;
%     model         = postMCMC.model;
%     wavespecsy    = postMCMC.wavespecsc;
%     MCMC_tau      = postMCMC.MCMC_tau;
%     MCMC_pi       = postMCMC.MCMC_pi;
%     MCMC_beta = postMCMC.MCMC_beta;
%     MCMC_alpha = postMCMC.MCMC_alpha;
%     MCMC_flag_theta = postMCMC.MCMC_flag_theta; 
%     MCMC_tau = postMCMC.MCMC_tau; 
%     MCMC_pi = postMCMC.MCMC_pi; 
%     MCMC_theta = postMCMC.MCMC_theta;
%     theta = postMCMC.theta;
%     model = postMCMC.model;
%     wavespecsc = postMCMC.wavespecsc;
%     wavespecsy = postMCMC.wavespecsy;
%     pcaspecsx = postMCMC.pcaspecsx; 
%     twosample = postMCMC.twosample; 
%     get_sigma = postMCMC.get_sigma; 
%     sampleU = postMCMC.sampleU;
    %%
    p           = model.p;
    nScalarCov  = model.nScalarCov; % MZ added 2/3/17
    ...
    delt        = model.delt;
    alf         = model.alf;
    stdXt        = model.stdXt; % MZ 5/4/19
    stdY         = model.stdY;  % MZ 5/4/19
    if twosample == 1;
        delt0   = model.delt0;
        delt1   = model.delt1;
    end;
...
%     meansx = col_mean_mat;
    ...
    % scaling of Y and Xt once mcmc samples are in data space - MZ 5/4/19
    ...
    if twosample == 0;
        Beta    = NaN(B,Tx*T); %previously Tx*Tx so 1000 x (9 exposure*100 sites)
        A       = NaN(B,T); % previously Tx so 1000 x 100 sites
        %BetaScalar = NaN(B,T); % MZ added 5/2/17, right now only handles single scalar covariate
        BetaScalar = NaN(B, T*nScalarCov); % MZ added 1/28/18 so 1000 x (100*nScalarCov)
        for i = 1:B;
            Betai   = reshape(MCMC_beta(i,:)',K,p)'; % p x K (because tranpose)
            
            %% Project back into Y space
            As          = Betai(1,:); % think this is intercept
            A1          = idwt_rows(As,wavespecsy);
            if nScalarCov > 0;
                betaSc   = Betai(2:(1+nScalarCov), :); % MZ 8/25/17 add 1 to account for intercept column
                betaSc_idwt = idwt_rows(betaSc, wavespecsy);               
                beta_temp = Betai((2+nScalarCov):end, :); %MZ 2/3/17 add 1 to account for intercept column
            ...
            A(i,:)      = A1;
            if nScalarCov > 0;
                %BetaScalar(i, :)  =  betaSc_idwt;
                BetaScalar(i, :) = reshape(betaSc_idwt, 1, T*nScalarCov); % MZ added 1/28/18                
            end;     
            ...
            if pcaspecsx.pca == 1;
                ...
                %temp                 = zeros(size(scorex,2),Tx); % changed to T, time in Y % MZ 9/17/18
                temp                = zeros(size(scorex,2),T); % changed to T, time in Y 
                temp(1:pcaCol,:)    = yidwt;
                Betai               = (temp'/coefx + meansx)';                
            end;
            if pcaspecsx.pca == 0; %% added MZ 9/17/18
                Betai = yidwt;
            end;
            if wavespecsc.compress == 1;
                temp                        = zeros(size(Betai',1),length(wavespecsc.keep));
  %             temp                        = zeros(size(Betai((2+nScalarCov):end, :)',1),length(wavespecsc.keep)); %MZ
  %             added 11/4/18
              temp(:,wavespecsc.keep==1)  = Betai';
  %              temp(:,wavespecsc.keep==1)  = Betai((2+nScalarCov):end, :)'; % MZ added 11/4/18
                Betai                       = temp;
            end;
...
%        Beta = BetaData; % replace unscaled version of surface with scaled version
    ...
    % Note from MZ: below pertains to two-sample case
    else
        Beta0   = NaN(B,Tx*Tx);
        Beta1   = NaN(B,Tx*Tx);
        Diff    = NaN(B,Tx*Tx);
        ...
        %% standardize the beta surface MZ added 10/27/18
        %dimProbe = find(size(Beta) == size(model.stdY, 2));
        %dimExp = find(size(Beta) == size(model.stdX, 2));
%         fprintf('\n dimProbe = %d \n', dimProbe);
%         fprintf('\n dimExp = %d \n', dimExp);
...
%    Note from MZ: below pertains to two sample case
    ...
    if twosample == 0;
        ...
        postout.Beta        = Beta; % MZ added 5/1/18 for purposes of running FDR sensitivities
%         postout.BetaData    = BetaData; % MZ added 5/5/2019 to check if rescaling worked 
        ...
        if nScalarCov > 0 ;
%             postout.Q025_betahatScalar   = quantile(BetaScalar,0.025); % added by MZ: 5/2/17
%             postout.Q975_betahatScalar   = quantile(BetaScalar,0.975); % added by MZ: 5/2/17
%             postout.betahatScalar = mean(BetaScalar); % added by MZ: 5/2/17
            postout.betahatScalar = reshape(mean(BetaScalar),nScalarCov, T); % added MZ 1/28/18
        end;
    else
        %% group 0 %%
