...
%%%% Save objects for troubleshooting in PostProcess steps
% postMCMC.MCMC_beta = MCMC_beta;
% postMCMC.MCMC_alpha = MCMC_alpha;
% postMCMC.MCMC_flag_theta = MCMC_flag_theta;
% postMCMC.MCMC_tau = MCMC_tau;
% postMCMC.MCMC_pi = MCMC_pi;
% postMCMC.MCMC_theta= MCMC_theta;
% postMCMC.theta = theta;
% postMCMC.model = model;
% postMCMC.wavespecsc = wavespecsc;
% postMCMC.wavespecsy = wavespecsy;
% postMCMC.pcaspecsx = pcaspecsx;
% postMCMC.twosample = twosample;
% postMCMC.get_sigma = get_sigma;
% postMCMC.sampleU = sampleU;

% objname           = sprintf('C:/Users/Michele/Dropbox/WFFR Epigenetics/Michele/Test Objects/postMCMC');
% save(objname, 'postMCMC');
...
%         PostProcess_inMem(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_theta,MCMC_tau,MCMC_pi,betans,theta,model,wavespecsy,get_sigma,sampleU,MCMC_U);
        postout = PostProcess_sim(MCMC_beta,MCMC_alpha,MCMC_flag_theta,MCMC_tau,MCMC_pi,MCMC_theta,theta,model,wavespecsc,wavespecsy,pcaspecsx,twosample,get_sigma,sampleU,MCMC_U);
    end
...
if length(model.delt) == 2;
    res.psiStrict   = postout.psiStrict;
    res.psiLax      = postout.psiLax;
end;
...
if twosample == 1;
    ...
    res.pwCI0       = postout.pwCI0;
    res.pwCI1       = postout.pwCI1;
    res.pwCId       = postout.pwCId;
    res.MAPs0       = postout.MAPs0;
    res.UMAPs0      = postout.UMAPs0;
    res.LMAPs0      = postout.LMAPs0;
    res.MAPs1       = postout.MAPs1;
    res.UMAPs1      = postout.UMAPs1;
    res.LMAPs1      = postout.LMAPs1;
    res.MAPsD       = postout.MAPsD;
    res.UMAPsD      = postout.UMAPsD;
    res.LMAPsD      = postout.LMAPsD;
else
