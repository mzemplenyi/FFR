function [ res ] = flmm_sim_staticCov2( Y, X, Z, FDR, SIMspecs, TSspecs, MCMCspecs, wavespecsx, wavespecsy, gbf, saveraw, pcaspecsx)
%function [ res ] = flmm(  )
%==========================================================================
...
%   Detailed explanation goes here
    ...
    %%%%%%%%%%%%%%%%%%5 added 5/4/19
    % Scale and center X so var = 1 at each time point
    
scaleX      = zeros(N,V);
stdX        = zeros(1,V); % store the std deviation of X for undoing standardization later
%scaleX      = zeros(size(simX,1),size(simX,2));
for i = 1:V,
    time_i          = X(:,i);
    tempStd         = std(time_i);
